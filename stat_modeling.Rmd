...
p1 <- plot_ly(merged, x = ~year, y = ~pm25_annual, color = ~borough,
              type = 'scatter', mode = 'lines+markers',
              colors = viridis(5),
              line = list(width = 2),
              marker = list(size = 5)) %>%
  layout(
    title = list(text = "<b>PM2.5 Over Time</b>", font = list(size = 15)),
    xaxis = list(title = "Year", titlefont = list(size = 12)),
    yaxis = list(title = "PM2.5 (μg/m³)", titlefont = list(size = 12)),
    showlegend = FALSE,
    margin = list(l = 60, r = 50, t = 50, b = 40)
  )

p2 <- plot_ly(merged, x = ~year, y = ~asthma_rate, color = ~borough,
              type = 'scatter', mode = 'lines+markers',
              colors = viridis(5, option = "plasma"),
              line = list(width = 2),
              marker = list(size = 5)) %>%
  layout(
    title = list(text = "<b>Asthma Rates Over Time</b>", font = list(size = 15)),
    xaxis = list(title = "Year", titlefont = list(size = 12)),
...
subplot(p1, p2, nrows = 2, shareX = FALSE, titleY = TRUE)
```
...
datatable(
  ...
  formatStyle(
    'AIC',
    background = styleColorBar(as.numeric(comparison$AIC), '#dbeafe'),
...
**Statistical Methods:**
- Linear Regression, Negative Binomial Regression, Interaction Models
- Software: R (tidyverse, MASS, broom)

**Data Sources:**
- PM2.5 Data: NYC Open Data
- Asthma ED Data: NYC Department of Health and Mental Hygiene
